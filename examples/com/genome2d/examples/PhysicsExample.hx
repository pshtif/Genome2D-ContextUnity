...
class PhysicsExample extends AbstractExample
{
    ...
    override public function initExample():Void {		
		...
		createWall(0,0,Screen.width,1);
		createWall(0,Screen.height,Screen.width,1);
		createWall(0,0,1,Screen.height);
		createWall(Screen.width,0,1,Screen.height);
		#end
		...
		for (i in 0...count) {
			// Create a sprite
			sprite = createSprite(50+(i%30)*40, 50+Math.floor(i/30)*40, "assets/atlas.png_"+Std.int(Math.random()*9));
		}
	...
	#elseif box2d
	private function update_handler(p_deltaTime:Float):Void {
		if (p_deltaTime > 0) world.step(p_deltaTime/1000, 10, 10);
		world.clearForces();

		accumulatedTime+= p_deltaTime;
		if (accumulatedTime > 1500) {
			accumulatedTime = 0;
			world.getGravity().x = -world.getGravity().x;
			world.getGravity().y = -world.getGravity().y;
		}
	...
	private function createWall(p_x:Float, p_y:Float, p_width:Float, p_height:Float):Void {
		var floorDef:B2BodyDef = new B2BodyDef();
		floorDef.position.set(p_x / physScale, p_y / physScale);
		var floorFix:B2PolygonShape = new B2PolygonShape();
		floorFix.setAsBox(p_width / physScale, p_height / physScale);
